...
/// <summary>
/// The GameBoy boot ROM, which is unloaded after boot.
/// </summary>
public class BootRom : IMemDevice
{
    /// <summary>
    /// The classic GameBoy boot ROM.
    /// </summary>
    private readonly byte[] m_dmgBootROM =
    [
    ...
    /// <summary>
    /// The color GameBoy boot ROM.
    /// </summary>
    // array size is 2304
    private readonly byte[] m_gbcBootROM =
    [
    ...
    private readonly byte[] m_realMem;
    private bool m_isLoaded = true;
    private GameBoyMode m_mode = GameBoyMode.Dmg;

    public ushort FromAddr => 0x0000;
    public ushort ToAddr => (ushort)(m_realMem.Length - 1);

    public BootRom()
    {
        m_realMem = new byte[Math.Max(m_dmgBootROM.Length, m_gbcBootROM.Length)];
    }

    public void SetMode(GameBoyMode mode) =>
        m_mode = mode;

    public void PrimeCartridgeData(byte[] romData)
    {
        Array.Clear(m_realMem);
        if (romData == null || romData.Length == 0)
            return;
        Array.Copy(romData, m_realMem, Math.Min(m_realMem.Length, romData.Length));
    }
    ...
    public byte Read8(ushort addr)
    {
        ...
        if (!m_isLoaded)
            return m_realMem[addr];

        if (m_mode == GameBoyMode.Dmg)
            return addr < m_dmgBootROM.Length ? m_dmgBootROM[addr] : m_realMem[addr];

        if (addr < 0x0100 || addr >= 0x0200)
            return addr < m_gbcBootROM.Length ? m_gbcBootROM[addr] : (byte)0xFF;
